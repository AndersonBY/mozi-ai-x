...
def generate_attribute_report(results: list[dict]):
    ...
    report_lines.append(
        f"- ⚠️ 属性不完整: {incomplete_classes}/{total_classes} ({incomplete_classes / total_classes * 100:.1f}%)\n"
    )
    ...
    for result in results:
        ...
        report_lines.append(
            f"**覆盖率**: {result['coverage']}% ({len(result['expected_attrs']) - len(result['missing_attrs'])}/{len(result['expected_attrs'])})\n"
        )
